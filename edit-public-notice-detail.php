...
<div class="px-6 py-4">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6" data-aos="fade-down">
            ...
            <nav class="flex mt-2" aria-label="Breadcrumb">
                <ol class="inline-flex items-center space-x-1 md:space-x-2">
                    ...
                    <li class="inline-flex items-center">
                        ...
                        <a href="manage-public-notice.php" class="text-sm font-medium text-gray-500 hover:text-primary">
                            Public Notices
                        </a>
                    </li>
        ...
        <a href="manage-public-notice.php" class="mt-4 md:mt-0 inline-flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden max-w-4xl mx-auto" data-aos="fade-up">
        <div class="px-6 py-4 border-b">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500">Update the notice details below</p>
                </div>
            </div>
        </div>
        ...
        <form class="p-6" method="post" onsubmit="return validateNoticeForm()">
            ...
            <div class="mb-6">
                ...
                ><?php echo htmlspecialchars($row->NoticeMessage); ?></textarea>
                <p class="mt-1 text-xs text-gray-500">This will be visible to all users</p>
            </div>
            
            <!-- Preview Section -->
            <div class="mb-6 p-4 border rounded-lg bg-gray-50">
                <h4 class="text-sm font-medium text-gray-700 mb-2">Notice Preview</h4>
                <div class="bg-white p-4 rounded-lg shadow-sm">
                    <h3 class="text-lg font-semibold text-gray-800 mb-2" id="preview-title"><?php echo htmlspecialchars($row->NoticeTitle); ?></h3>
                    <div class="text-sm text-gray-600" id="preview-message"><?php echo nl2br(htmlspecialchars($row->NoticeMessage)); ?></div>
                    <div class="mt-3 text-xs text-gray-500">Posted on: <?php echo date('M d, Y'); ?></div>
                </div>
            </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Get form elements
        const titleInput = document.getElementById('nottitle');
        const msgInput = document.getElementById('notmsg');
        ...
        msgInput.addEventListener('input', function() {
            previewMsg.innerHTML = this.value.replace(/\n/g, '<br>');
        });
    });
    ...
    function validateNoticeForm() {
        ...
        const message = document.getElementById('notmsg').value.trim();
        ...
        if (message.length === 0) {
            Swal.fire({
                ...
                text: 'Please enter notice message content',
                showConfirmButton: true
            });
            return false;
        }
